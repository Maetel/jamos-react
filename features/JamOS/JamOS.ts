...
import {
  ...
  openToolbar,
  selectForceHideDock,
  selectForceHideToolbar,
  selectForceOpenDock,
  selectForceOpenToolbar,
  selectIsDockOpen,
  selectIsToolbarOpen,
  selectNotifDuration,
  selectNotifs,
...
export default class JamOS {
  ...
  public static get procmgr() {
    return ProcMgr.getInstance();
  }
  public static get filemgr() {
    return FileMgr.getInstance();
  }
  public static get setmgr() {
    return SetMgr.getInstance();
  }
  public static get theme(): Theme {
    return SetMgr.getInstance().themeReadable();
  }
  ...
  public static setNotif(
    ...
    if (
      notifsVal.length > 0 &&
      Math.abs(lastElem?.timestamp - now) < 50 &&
      lastElem?.msg === msg
    ) {
  ...
  public static async loadFromString(data: string) {
    ...
    const cbs = {
      proc: async (_data) => {
        await ProcMgr.getInstance().loadFromString(_data);
      },
      files: async (_data) => {
        await FileMgr.getInstance().loadFromString(_data);
      },
      settings: async (_data) => {
        await SetMgr.getInstance().loadFromString(_data);
      },
      os: async (_data) => {
        await store.dispatch(loadOsFromString(_data));
      },
    };
    try {
  ...
  public static openContextMenu(
    ...
    items: string[],
    callbacks: (() => void)[],
    args?: { onMount?: string; onDestroy?: string }
  ) {
  ...
  public static get minIdLength() {
    return 3;
  }
  public static get maxIdLength() {
    return 12;
  }
  public static get minPasswordLength() {
    return 3;
  }
  public static get maxPasswordLength() {
    return 12;
  }

  public static isUserInfoValid(
    userId: string,
    password: string,
    onError?: (msg: string) => void
  ): boolean {
    if (
      !userId ||
      userId.length < JamOS.minIdLength ||
      userId.length > JamOS.maxIdLength
    ) {
      onError?.(
        `User id is not correct. Id must have at least ${JamOS.minIdLength} characters and cannot be more than ${JamOS.maxIdLength}.`
      );
      return false;
    }
    if (
      !password ||
      password.length < JamOS.minIdLength ||
      password.length > JamOS.maxIdLength
    ) {
      onError?.(
        `Password is not correct. Password must have at least ${JamOS.minPasswordLength} characters and cannot be more than ${JamOS.maxPasswordLength}.`
      );
      return false;
    }
    return true;
  ...
  public static createWorld(
    ...
    axios
      ...
      .then((res) => {
        ...
        if (cont) {
          args?.onSuccess?.(
            `Successfully created world ${cont.wid} for ${cont.uid}`
          );
        } else {
  ...
  public static worldList(procId?: string) {
    ...
    procId =
      ...
      JamOS.procmgr
        .processesValue()
        ?.find((proc) => proc.type === "worldeditor")?.id;
    if (!procId) {
      return;
    }
    ...
    axios
      ...
      .then((res) => {
        const worldList: WorldInfo[] = res.data?.content?.rows?.map(
          (datum) => ({
            uid: datum.uid,
            wid: datum.wid,
            rights: datum.wid,
  ...
  public static trySignup(
    userId: string,
    password: string,
    args?: {
      signinAfterSignup?: boolean;
      procId?: string;
      onSignup?: (msg: string) => void;
      onError?: (msg: string) => void;
    }
  ): void {
    if (!JamOS.isUserInfoValid(userId, password, args?.onError)) {
    ...
    axios
      ...
      .then((res) => {
        ...
        if (content && wid && wid === content.wid) {
          ...
          JamOS.setNotif(
            `Failed to delete ${wid}${res.data?.content ?? ""}`,
            "error"
          );
        }
        JamOS.setLoading(false);
  ...
  public static saveWorld(type: SaveWorldType = "whole") {
    ...
    axios
      .post(this.apis.worldSave, payload, this.authHeader)
      .then((res) => {
        // console.log(res);
        JamOS.setLoading(false);
  ...
  public static loadWorld(wid?: string) {
    ...
    if (wid === "sample_world") {
      console.log("Loadworld : sample_world");
      return fetch("/sample_world.json")
        .then((val) => val.json())
        .then((json) => jsonHandler(json.content))
        .catch(errHandler);
    }

    return axios
      .get(this.apis.worldLoad + wid, this.authHeader)
      .then((res) => jsonHandler(res.data?.content))
      .catch(errHandler);
  }
  ...
  public static async refreshToken() {
    ...
    return axios
      .get(JamOS.apis.refreshToken, config)
      .then((res) => {
